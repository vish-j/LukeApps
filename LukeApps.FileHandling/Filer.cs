using LukeApps.FileHandling.DAL;
using LukeApps.FileHandling.Interfaces;
using LukeApps.FileHandling.Models;
using LukeApps.TrackingExtended;
using System;
using System.Collections.Generic;
using System.Configuration;
using System.IO;
using System.IO.Compression;
using System.Linq;
using System.Text.RegularExpressions;
using System.Web;

namespace LukeApps.FileHandling
{
    public class Filer : ICanAddFilesOrFileCategories, ICanGetSetFileCategory, ICanSaveChanges, ICanUpload, ICanLog, ICanDownload, ICanDownloadMultiple, ICanEdit, IDisposable
    {
        #region Parameters

        private FileStoreEntities db;
        private bool _isCurrentFolderSet = false;
        private string _username;
        private string _fileRootPath;
        private Folder _folder;
        private IEnumerable<FilerInputStream> _files;

        private List<FileRecord> _fileStore;
        private List<Guid> _keys = new List<Guid>();

        #endregion Parameters

        #region Constructors

        private Filer(string username)
        {
            db = new FileStoreEntities(username);
            setFileRootPath();
            _username = username;
        }

        private Filer(Guid Key)
        {
            db = new FileStoreEntities("");
            setFileRootPath();
            _keys.Add(Key);
        }

        private Filer(List<Guid> Keys)
        {
            db = new FileStoreEntities("");
            setFileRootPath();
            _keys = Keys;
        }

        private Filer(Guid Key, string username)
        {
            db = new FileStoreEntities(username);
            setFileRootPath();
            _keys.Add(Key);
        }

        private Filer(List<Guid> Keys, string username)
        {
            db = new FileStoreEntities(username);
            setFileRootPath();
            _keys = Keys;
        }

        #endregion Constructors

        #region Properties

        public static FileDownload InvalidDefaultFile => getInvalidFile();

        #endregion Properties

        #region Exposed Methods

        /// <summary>
        /// Set the File Key that is needed to Download the respective file
        /// </summary>
        /// <param name="Key"></param>
        /// <returns></returns>
        public static ICanDownload SetFileKey(Guid Key)
        {
            return new Filer(Key);
        }

        public static ICanEdit SetFileKey(Guid Key, string UserToEdit)
        {
            return new Filer(Key, UserToEdit);
        }

        /// <summary>
        /// Set the File Key that is needed to Download the respective file
        /// </summary>
        /// <param name="Key"></param>
        /// <returns></returns>
        public static ICanDownload SetFileKey(string Key)
        {
            return new Filer(cleanGuidList(Key));
        }

        public static ICanEdit SetFileKey(string Key, string UserToEdit)
        {
            return new Filer(cleanGuidList(Key), UserToEdit);
        }

        private static List<Guid> cleanGuidList(string Key)
        {
            var GuidList = new List<Guid>();
            try
            {
                GuidList.Add(new Guid(Key));
            }
            catch
            {
                GuidList = new List<Guid>();
            }

            return GuidList;
        }

        /// <summary>
        /// Set the File Keys that is needed to Download the respective file
        /// </summary>
        /// <param name="Keys"></param>
        /// <returns></returns>
        public static ICanDownloadMultiple SetFileKeys(List<Guid> Keys)
        {
            return new Filer(Keys);
        }

        /// <summary>
        /// Set the File Keys that is needed to Download the respective file
        /// </summary>
        /// <param name="Keys"></param>
        /// <returns></returns>
        public static ICanDownloadMultiple SetFileKeys(string Keys)
        {
            var GuidList = new List<Guid>();
            try
            {
                var guids = Keys.Split(',');

                foreach (var strGuid in guids)
                {
                    if (!(string.IsNullOrEmpty(strGuid)))
                        GuidList.Add(new Guid(strGuid));
                }
            }
            catch
            {
                GuidList = new List<Guid>();
            }
            return new Filer(GuidList);
        }

        /// <summary>
        /// Set the Username in Context in order to upload the files
        /// </summary>
        /// <param name="Username"></param>
        /// <returns></returns>
        public static ICanAddFilesOrFileCategories SetUsername(string Username)
        {
            return new Filer(Username);
        }

        public FileInformation GetFileInformation()
        {
            return GetFilesInformation().FirstOrDefault();
        }

        public IEnumerable<FileInformation> GetFilesInformation()
        {
            IEnumerable<FileRecord> files;
            if (_fileStore == null)
            {
                var keysArray = _keys.ToArray();
                files = db.FileStore.Include("Folder")
                   .Where(f => keysArray.Contains(f.FileRecordID))
                   .AsEnumerable();
            }
            else
            {
                files = _fileStore;
            }

            foreach (var f in files)
            {
                yield return new FileInformation
                {
                    FileName = f.FileName,
                    FileCode = f.FileRecordID.ToString(),
                    Size = f.ContentLength / 1024,
                    FileType = f.ContentType,
                    FileCategory = new FileCategory
                    {
                        ID = f.FolderID,
                        FileCategoryName = f.Folder.FolderName,
                        FileCategoryDisplayName = f.Folder.FolderDisplayName
                    }
                };
            }
        }

        /// <summary>
        /// Add files in order to upload them
        /// </summary>
        /// <param name="webFiles"></param>
        /// <returns></returns>
        public ICanUpload AddFiles(HttpFileCollectionBase webFiles)
        {
            _files = getFilerStreams(webFiles);
            return this;
        }

        /// <summary>
        /// Add files in order to upload them
        /// </summary>
        /// <param name="Files"></param>
        /// <returns></returns>
        public ICanUpload AddFile(string fileName, Stream inputStream, string fileCategoryKey, string contentType)
        {
            _files = new List<FilerInputStream>
                    {
                        new FilerInputStream {
                            FileName = fileName,
                            ContentType = contentType,
                            FileCategoryKey = fileCategoryKey,
                            FInputStream = inputStream
                        }
                    };
            return this;
        }

        /// <summary>
        /// Upload the files
        /// </summary>
        /// <returns></returns>
        public ICanLog Upload()
        {
            var fileStore = upload().ToList();
            if (_fileStore == null || !_fileStore.Any())
            {
                db.FileStore.AddRange(fileStore);
                db.SaveChanges();
                _fileStore = fileStore;
            }
            return this;
        }

        /// <summary>
        /// Call the function to download the file
        /// </summary>
        /// <returns></returns>
        public IEnumerable<FileDownload> DownloadMultiple()
        {
            if (_keys.Count() != 0)
            {
                IEnumerable<FileRecord> fileRecords = db.FileStore.Where(f => _keys.Contains(f.FileRecordID)).AsEnumerable();
                foreach (var fileRecord in fileRecords)
                {
                    if (fileRecord == null)
                    {
                        yield return getInvalidFile();
                    }
                    else
                    {
                        string path = resolveFolderpath(fileRecord.FolderID);
                        byte[] content;
                        bool IsNA = false;
                        try
                        {
                            content = getFile(path, fileRecord.FileName, fileRecord.SerialNumber, fileRecord.AuditDetail.CreatedDate);
                        }
                        catch (Exception)
                        {
                            content = naFile();
                            IsNA = true;
                        }

                        yield return new FileDownload
                        {
                            FileName = IsNA ? "Invalid.png" : fileRecord.FileName,
                            ContentType = IsNA ? "image/png" : fileRecord.ContentType,
                            FileContent = content
                        };
                    }
                }
            }
            else
            {
                yield return getInvalidFile();
            }
        }

        public FileDownload DownloadZipped(bool isUnZippedIfOneKey = false)
        {
            if (_keys.Count() != 0)
            {
                if (isUnZippedIfOneKey && _keys.Count() == 1)
                    return Download();

                IEnumerable<FileRecord> fileRecords = db.FileStore.Where(f => _keys.Contains(f.FileRecordID)).AsEnumerable();
                byte[] zipBytes = null;

                using (var compressedFileStream = new MemoryStream())
                {
                    using (ZipArchive zipArchive = new ZipArchive(compressedFileStream, ZipArchiveMode.Create, leaveOpen: true))
                    {
                        foreach (var fr in fileRecords)
                        {
                            byte[] content;
                            bool isNA = false;
                            string path;
                            if (fr == null)
                            {
                                content = naFile();
                                isNA = true;
                            }
                            else
                            {
                                path = resolveFolderpath(fr.FolderID);
                                try
                                {
                                    content = getFile(path, fr.FileName, fr.SerialNumber, fr.AuditDetail.CreatedDate);
                                }
                                catch (Exception)
                                {
                                    content = naFile();
                                    isNA = true;
                                }
                            }

                            // Create the instance of the file.
                            var zipEntry = zipArchive.CreateEntry(isNA ? "Invalid.png" : fr.FileName);

                            // Get the stream of the file.
                            using (var entryStream = new MemoryStream(content))

                            // Get the Stream of the zipEntry
                            using (var zipEntryStream = zipEntry.Open())
                            {
                                // Adding the file to the zip file.
                                entryStream.CopyTo(zipEntryStream);
                            }
                        }
                    }
                    zipBytes = compressedFileStream.ToArray();
                }
                return new FileDownload
                {
                    FileName = "files.zip",
                    ContentType = System.Net.Mime.MediaTypeNames.Application.Octet,
                    FileContent = zipBytes
                };
            }
            else
            {
                return getInvalidFile();
            }
        }

        public bool SetFileCategory(string category)
        {
            var key = _keys.First();
            var fileRecord = db.FileStore.Find(key);
            var oldFolder = getFolder(fileRecord.FolderID);
            var newFolder = getFolder(category);
            fileRecord.FolderID = newFolder.FolderID;

            string fileName = processFileName(fileRecord.FileName, fileRecord.SerialNumber, fileRecord.AuditDetail.CreatedDate);
            string sourcePath = resolveFolderpath(oldFolder);
            string targetPath = resolveFolderpath(newFolder);

            string sourceFile = Path.Combine(sourcePath, fileName);
            string destFile = Path.Combine(targetPath, fileName);

            if (!Directory.Exists(targetPath))
            {
                Directory.CreateDirectory(targetPath);
            }

            File.Move(sourceFile, destFile);

            db.Entry(fileRecord).State = System.Data.Entity.EntityState.Modified;
            return db.SaveChanges() > 0;
        }

        public ICanLog MoveFiles(string keys)
        {
            //_fileStore =
            try
            {
                // from = System.IO.Path.Combine(@"E:\vid\", "(" + i.ToString() + ").PNG");
                // to = System.IO.Path.Combine(@"E:\ConvertedFiles\", i.ToString().PadLeft(6, '0') + ".png");

                // File.Move(from, to); // Try to move
                Console.WriteLine("Moved"); // Success
            }
            catch (IOException ex)
            {
                Console.WriteLine(ex); // Write error
            }
            return this;
        }

        public ICanSaveChanges RenameFile(string newname)
        {
            FileRecord fileRecord = db.FileStore.Find(_keys.First());
            string fpath = resolveFolderpath(fileRecord.FolderID);

            try
            {
                File.Move(getFilePath(fpath, fileRecord.FileName, fileRecord.SerialNumber, fileRecord.AuditDetail.CreatedDate),
                    getFilePath(fpath, newname, fileRecord.SerialNumber, fileRecord.AuditDetail.CreatedDate));
                fileRecord.FileName = newname;
                db.Entry(fileRecord).State = System.Data.Entity.EntityState.Modified;
            }
            catch (Exception)
            {
            }




            return this;
        }

        public FileDownload Download()
        {
            if (_keys.Count() != 0)
            {
                foreach (var key in _keys)
                {
                    FileRecord fileRecord = db.FileStore.Find(key);
                    if (fileRecord == null)
                    {
                        return getInvalidFile();
                    }
                    else
                    {
                        string path = resolveFolderpath(fileRecord.FolderID);
                        try
                        {
                            return new FileDownload
                            {
                                FileName = fileRecord.FileName,
                                ContentType = fileRecord.ContentType,
                                FileContent = getFile(path, fileRecord.FileName, fileRecord.SerialNumber, fileRecord.AuditDetail.CreatedDate)
                            };
                        }
                        catch
                        {
                            return getInvalidFile();
                        }
                    }
                }
            };
            return getInvalidFile();
        }

        /// <summary>
        /// Get the File Keys of the uploaded files
        /// </summary>
        /// <returns></returns>
        public string GetKeys()
        {
            var keys = getKeys();
            if (keys == null || !keys.Any())
            {
                return null;
            }
            else
            {
                return string.Join(",", keys);
            }
        }

        /// <summary>
        /// Get the File Key of the uploaded file
        /// </summary>
        /// <returns></returns>
        public string GetKey()
        {
            var keys = getKeys();
            var keyCount = keys.Count();

            switch (keyCount)
            {
                case 0:
                    return null;

                case 1:
                    return keys.First().ToString();

                default:
                    throw new InvalidOperationException("More than one Key");
            }
        }

        public IEnumerable<KeyValuePair<Guid, string>> GetKeysWithCategory()
        {
            return getKeysWithCategory();
        }

        public ICanGetSetFileCategory SetCurrentFolder(long folderID)
        {
            _folder = getFolder(folderID);
            return this;
        }

        public ICanGetSetFileCategory SetCurrentFolder(string folderName)
        {
            _isCurrentFolderSet = true;
            _folder = getFolder(folderName);
            return this;
        }

        public List<FileCategory> GetFileCategories()
        {
            return db.Folders.Where(f => f.ParentFolder.FolderID == _folder.FolderID).ToList()
                 .Select(f => new FileCategory
                 {
                     ID = f.FolderID,
                     FileCategoryName = f.FolderName,
                     FileCategoryDisplayName = f.FolderDisplayName
                 })
                 .ToList();
        }

        public ICanSaveChanges AddNewFileCategory(string FileCategory)
        {
            Regex rgx = new Regex("[^a-zA-Z0-9-]");
            string FileCategoryName = rgx.Replace(FileCategory, "");
            db.Folders.Add(new Folder()
            {
                FolderName = FileCategoryName,
                ParentFolder = _folder,
                FolderDisplayName = FileCategory,
                AuditDetail = new AuditDetail { CreatedEntryUserID = _username }
            });
            return this;
        }

        public int SaveChanges()
        {
            return db.SaveChanges();
        }

        #endregion Exposed Methods

        #region private methods

        private static byte[] invalid;

        /// <summary>
        /// "Not Available File" with metadata
        /// </summary>
        /// <returns>Bytes</returns>
        private static FileDownload getInvalidFile()
        {
            return new FileDownload { FileName = "InvaildRequest.png", ContentType = "image/png", FileContent = naFile() };
        }

        /// <summary>
        /// "Not Available File" is Rendered and Returned
        /// </summary>
        /// <returns>Bytes</returns>
        private static byte[] naFile()
        {
            if (invalid == null)
                invalid = System.Convert.FromBase64String("");

            return invalid;
        }

        private IEnumerable<Guid> getKeys()
        {
            return _fileStore.Select(c => c.FileRecordID);
        }

        private IEnumerable<KeyValuePair<Guid, string>> getKeysWithCategory()
        {
            return _fileStore.Select(c => new KeyValuePair<Guid, string>(c.FileRecordID, c.Folder.FolderName));
        }

        private void setFileRootPath()
        {
            _fileRootPath = ConfigurationManager.AppSettings["FileRootPath"];
        }

        private IQueryable<Folder> getFolders()
        {
            return db.Folders.Include("ParentFolder");
        }

        private Folder getFolder(string folderKey)
        {
            return getFolders().Where(f => f.FolderName == folderKey).FirstOrDefault();
        }

        private Folder getFolder(long? id)
        {
            return getFolders().Where(f => f.FolderID == id).FirstOrDefault();
        }

        private string resolveFolderpath(string folderKey)
        {
            if (_folder == null || _folder.FolderName != folderKey)
                _folder = getFolder(folderKey);

            return resolveFolderpath(_folder);
        }

        private string resolveFolderpath(long? id)
        {
            if (id != null)
            {
                _folder = getFolder(id);
                return resolveFolderpath(_folder);
            }
            else
            {
                this.Dispose(true);
                throw new ArgumentNullException(nameof(id), "No ID");
            }
        }

        private string resolveFolderpath(Folder folder)
        {
            if (folder == null)
            {
                this.Dispose(true);
                throw new ArgumentException("Bad Path", "path");
            }
            else
            {
                return _fileRootPath + folder.FullPath;
            }
        }

        private IEnumerable<FilerInputStream> getFilerStreams(HttpFileCollectionBase webFileCollection)
        {
            var arrFile = webFileCollection.AllKeys;

            for (int i = 0; i < arrFile.Length; i++)
            {
                var fileBase = webFileCollection[i];

                if (fileBase.ContentLength > 0)
                {
                    string fileName = Path.GetFileName(fileBase.FileName);

                    yield return new FilerInputStream
                    {
                        FileName = fileName,
                        FileCategoryKey = arrFile[i],
                        ContentType = fileBase.ContentType,
                        FInputStream = fileBase.InputStream,
                        ContentLength = fileBase.ContentLength
                    };
                }
            }
        }

        private IEnumerable<FileRecord> upload()
        {
            int cntr = 0;

            foreach (var item in _files)
            {
                DateTime fileDate = DateTime.Now;
                cntr++;
                string folderpath = _isCurrentFolderSet ? resolveFolderpath(_folder) : resolveFolderpath(item.FileCategoryKey);
                bool isExists = Directory.Exists(folderpath);
                if (!isExists)
                    Directory.CreateDirectory(folderpath);
                var path = getFilePath(folderpath, item.FileName, cntr, fileDate);
                using (var fileStream = File.Create(path))
                {
                    item.FInputStream.Seek(0, SeekOrigin.Begin);
                    item.FInputStream.CopyTo(fileStream);
                }

                yield return new FileRecord(_username)
                {
                    FileName = item.FileName,
                    ContentType = item.ContentType,
                    ContentLength = item.ContentLength,
                    FolderID = _folder.FolderID,
                    SerialNumber = cntr,
                    AuditDetail = new AuditDetail { CreatedDate = fileDate, CreatedEntryUserID = _username }
                };
            }
        }

        /// <summary>
        /// Fetches the Requested File from the Filer.
        /// </summary>
        /// <param name="folderpath">FolderPath of the Needed File</param>
        /// <param name="fileName">Name of the Needed File</param>
        /// <param name="serialNo">Sequence</param>
        /// <param name="createdDate">Created Date of File</param>
        /// <returns>Bytes</returns>
        /// <exception cref="ArgumentException"></exception>
        /// <exception cref="ArgumentNullException"></exception>
        private byte[] getFile(string folderpath, string fileName, int serialNo, DateTime createdDate) =>
            getFile(getFilePath(folderpath, fileName, serialNo, createdDate));

        private byte[] getFile(string fullFileName) => File.ReadAllBytes(fullFileName);

        private static string getFilePath(string folderpath, string fileName, int serialNo, DateTime fileDate) =>
             Path.Combine(folderpath, processFileName(fileName, serialNo, fileDate));

        private static string processFileName(string fileName, int serialNo, DateTime fileDate) =>
            $"{fileName}-{serialNo:00}-{fileDate:yyyyMMddHHmmss}";

        #endregion private methods

        #region disposeable

        private bool disposedValue = false; // To detect redundant calls

        public void Dispose()
        {
            Dispose(true);
        }

        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    db.Dispose();
                }
                _username = null;
                _files = null;
                _folder = null;
                _fileStore = null;
                _keys = null;
                disposedValue = true;
            }
        }

        #endregion disposeable
    }
}